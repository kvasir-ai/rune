---
name: writing-plans
description: Generates a strict, atomic, step-by-step TDD implementation plan based on a design document. Optimized for automated execution.
---
...
- No circular dependencies (if t1 depends on t2, t2 must NOT depend on t1)
...
- If critical path > 70% of total tasks, note: "Sequential execution recommended — use executing-plans"
...
```bash
git add path/to/file_test.go path/to/file.go
...
**If plan has NO DAG annotations:**
- Use `executing-plans` to execute the plan sequentially in the current session
