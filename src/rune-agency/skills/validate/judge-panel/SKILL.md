...
REQUIRED OWNER: [agent or HITL]
NEXT PHASE: [Explore / Plan / Build / Validate / HITL]
UNBLOCK CONDITION: [required when BLOCKED]
```
...
| Safety | APPROVED WITH WARNINGS | ... |
| ... | ... | ... |

RECOMMENDATION: [single clear action for the user]
REQUIRED OWNER: [single owner]
NEXT PHASE: [Explore / Plan / Build / Validate / HITL]
UNBLOCK CONDITION: [required when BLOCKED]
```
...
- A single `BLOCKED` from any judge escalates to `BLOCKED` unless the synthesis can prove the finding is unsound
