...
def test_site_generator_emits_valid_search_result_markup() -> None:
    ...
    assert '--content-max: clamp(20rem, 74vw, 78ch);' in styles
    assert '--content-max-mobile: clamp(18rem, 100vw, 72ch);' in styles
    assert '.content { width: min(100%, var(--content-max)); max-width: none; min-width: 0; margin-inline: auto; padding: var(--content-pad-y) var(--content-pad-x) var(--content-pad-bottom); }' in styles
    ...
    assert "code { overflow-wrap: anywhere; word-break: normal; }" in styles
